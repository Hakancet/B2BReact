...
function RenderLabelStock({ props }: { props: Object }) {
  ...
  // console.log(variant);
  const { t } = useTranslation(lang, 'common');
  const { id, count, product_type } = data ?? {};
  const { isInCart, isInStock } = useCart();
  const outOfStock = isInCart(id) && !isInStock(id);

  if (Number(count) < 1 || outOfStock) {
    return (
    ...
    >
      <div className="relative flex-shrink-0 ">
        <div className="relative card-img-container overflow-hidden flex item-center w-full">
         <img src='assets/images/products/sunucu.jpeg' alt={name || 'Product Image'} className='w-[180px] h-[180px]'
            />
         {/*  <Image
            src={
              Array.isArray(image) && image.length > 0
                ? isValidUrl(image[0]) // İlk öğe geçerli bir URL ise
                  ? `http://crm.networkkurumsal.com:3001${image[0]}` // İlk öğeyi kullan
                  : productPlaceholder // İlk öğe geçerli bir URL değilse varsayılan görseli kullan
                : isValidUrl(image) // image dize değilse ve geçerli bir URL ise
                  ? `http://crm.networkkurumsal.com:3001${image}` // image'i kullan
                  : productPlaceholder // image dize değilse veya geçerli bir URL değilse varsayılan görseli kullan
            }
      ...
      <div className="flex flex-col h-full overflow-hidden relative product-cart-content">
        ...
        <div className="space-s-2">
          <span className="inline-block font-semibold text-[18px] text-brand">
            {product_type === 'variable' ? ` - ${maxPrice}` : price}
          </span>
          {basePrice && (
